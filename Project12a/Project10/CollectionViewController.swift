...
class CollectionViewController: UICollectionViewController {
    ...
    func savePeopleInUserDefaults() {
        let defaults = UserDefaults.standard
        guard let savedPeopleData = try? NSKeyedArchiver.archivedData(withRootObject: people,
                                                                requiringSecureCoding: false) else { return }
        
        defaults.set(savedPeopleData, forKey: "people")
    }
    
    func fetchPeopleFromUserDefaults() {
        let defaults = UserDefaults.standard
        guard let savedPeopleData = defaults.object(forKey: "people") as? Data else { return }
        guard let decodedPeople = try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(savedPeopleData) as? [Person] else { return }
